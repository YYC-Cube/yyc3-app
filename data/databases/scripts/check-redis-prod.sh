...
# 增强的安全校验参数
ENABLE_ENHANCED_SECURITY="${ENABLE_ENHANCED_SECURITY:-1}"  # 是否启用增强安全检查
...
if [[ "$ALLOW_WEAK_PROD" != "1" ]]; then
  ...
  # 增强安全模式下的额外检查
  if [[ "$ENABLE_ENHANCED_SECURITY" == "1" ]]; then
    echo "🔍 执行增强安全检查..."
    
    # 检查敏感命令是否被重命名或禁用
    SENSITIVE_COMMANDS=("CONFIG" "DEBUG" "SLAVEOF" "REPLICAOF" "MODULE")
    for cmd in "${SENSITIVE_COMMANDS[@]}"; do
      if ! grep -q "rename-command $cmd" "$CONF_PROD"; then
        echo "❌ 错误：增强安全模式下，敏感命令 $cmd 必须被重命名或禁用"
        exit 1
      fi
    done
