...
参考 [视觉MCP服务器](../mcp/vision–mcp–server) 和 [搜索MCP服务器](../mcp/search–mcp–server) 文档，配置完成后即可在 OpenCode 中使用。
