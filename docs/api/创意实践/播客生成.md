...
async def text_to_speech(text: str, background_tasks: BackgroundTasks, voice: str):
    ...
    if voice == "host":
params = {'tok': get_access_token(), 'tex': quote_plus(text), 'per': PER, 'spd': SPD, 'pit': PIT, 'vol':
VOL, 'aue': AUE, 'cuid': CUID, 'lan': 'zh', 'ctp': 1} # lan ctp 固定参数
    else:
params = {'tok': get_access_token(), 'tex': quote_plus(text), 'per': PER_Guest, 'spd': SPD, 'pit': PIT, 'vol':
VOL, 'aue': AUE, 'cuid': CUID, 'lan': 'zh', 'ctp': 1} # lan ctp 固定参数
    headers = {
...
# 第一次 LLM 请求
    data = {
        ...
        'messages': [
            ...
            {'role': 'user', 'content':
f"请将以下内容转换成播客对话,对话内容content加身份前缀,这是一个包含多个对象的JSON数组，每个对象都有两个键值对，分别是role（表示角色）和content（表示内容）。内容如下:\n{text_content}"}
        ]
    }
