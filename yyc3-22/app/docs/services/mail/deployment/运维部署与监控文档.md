...
**目标**: 构建高可用、可扩展、安全的邮件服务平台  
**部署环境**: Docker + Kubernetes + Cloud Native  
**监控体系**: Prometheus + Grafana + ELK Stack  
**CI/CD**: GitHub Actions + ArgoCD  

### 🎯 部署策略

1. **多环境管理**: Development → Staging → Production
2. **容器化部署**: 全应用Docker化，支持快速扩容
3. **自动化部署**: CI/CD流水线，零停机更新
4. **多可用区部署**: 保障高可用性和容灾能力
5. **监控全覆盖**: 应用指标、系统指标、业务指标
...
services:
  ...
  postgres:
    ...
    container_name: yyc3-mail-postgres-dev
    environment:
  ...
  redis:
    ...
    container_name: yyc3-mail-redis-dev
  ...
  backend:
    ...
    container_name: yyc3-mail-backend-dev
    environment:
  ...
  frontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile.dev
    container_name: yyc3-mail-frontend-dev
    environment:
  ...
  mailhog:
    ...
    container_name: yyc3-mail-mailhog-dev
    ports:
      - "1025:1025"  # SMTP服务器
      - "8025:8025"  # Web界面
    networks:
  ...
  minio:
    ...
    container_name: yyc3-mail-minio-dev
  ...
  prometheus:
    ...
    container_name: yyc3-mail-prometheus-dev
    command:
  ...
  grafana:
    ...
    container_name: yyc3-mail-grafana-dev
    environment:
...
services:
  ...
  postgres-master:
    image: postgres:15-alpine
    container_name: yyc3-mail-postgres-master
    environment:
      POSTGRES_DB: email_platform
  ...
  postgres-replica:
    image: postgres:15-alpine
    container_name: yyc3-mail-postgres-replica
    environment:
      POSTGRES_USER: ${POSTGRES_USER}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}
      PGUSER: postgres
  ...
  redis-node-1:
    image: redis:7-alpine
    container_name: yyc3-mail-redis-1
  ...
  redis-node-2:
    image: redis:7-alpine
    container_name: yyc3-mail-redis-2
  ...
  redis-node-3:
    image: redis:7-alpine
    container_name: yyc3-mail-redis-3
  ...
  backend-1:
    ...
    container_name: yyc3-mail-backend-1
    environment:
      NODE_ENV: production
  ...
  frontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile
    container_name: yyc3-mail-frontend
    environment:
      NGINX_HOST: ${DOMAIN}
      NGINX_PORT: 80
    networks:
  ...
  nginx:
    image: nginx:alpine
    container_name: yyc3-mail-nginx
    ports:
      - "80:80"
      - "443:443"
    volumes:
  ...
  minio:
    image: minio/minio:latest
    container_name: yyc3-mail-minio
    command: server /data --console-address ":9001"
    environment:
  ...
  elasticsearch:
    ...
    container_name: yyc3-mail-elasticsearch
    environment:
  ...
  logstash:
    ...
    container_name: yyc3-mail-logstash
    volumes:
  ...
  kibana:
    image: kibana:8.8.0
    container_name: yyc3-mail-kibana
    environment:
  ...
  prometheus:
    image: prom/prometheus:latest
    container_name: yyc3-mail-prometheus
    command:
      - '--config.file=/etc/prometheus/prometheus.yml'
      - '--storage.tsdb.path=/prometheus'
  ...
  grafana:
    image: grafana/grafana:latest
    container_name: yyc3-mail-grafana
    environment:
...
data:
  nginx.conf: |
    ...
    http {
        upstream backend {
            least_conn;
            server backend-service.yyc3-platform.svc.cluster.local:3000;
        }
...
data:
  tls.crt: LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0...  # Base64编码的证书
  tls.key: LS0tLS1CRUdJTiBQUklWQVRFIEtFWS0...  # Base64编码的私钥
```
    ...
    spec:
      ...
      - name: backend
        ...
        - name: REDIS_CLUSTER_NODES
          value: "redis-cluster-0.redis-headless.yyc3-platform.svc.cluster.local:6379,redis-cluster-1.redis-headless.yyc3-platform.svc.cluster.local:6379,redis-cluster-2.redis-headless.yyc3-platform.svc.cluster.local:6379"
        - name: JWT_SECRET
          valueFrom:
            secretKeyRef:
              name: yyc3-mail-app-secrets
...
jobs:
  ...
  deploy-staging:
    ...
    - name: Update kubeconfig
      run: |
        aws eks update-kubeconfig --region us-west-2 --name yyc3-platform-staging
    ...
    - name: Deploy to staging
      run: |
        ...
        # 等待部署完成
        kubectl rollout status deployment/backend-service -n yyc3-platform-staging
        kubectl rollout status deployment/frontend-service -n yyc3-platform-staging
    ...
    - name: Run smoke tests
      run: |
        ...
        # 健康检查
        kubectl get pods -n yyc3-platform-staging
  ...
  deploy-production:
    ...
    - name: Update kubeconfig
      run: |
        aws eks update-kubeconfig --region us-west-2 --name yyc3-platform-production

    - name: Create backup
      run: |
        # 数据库备份
        kubectl exec -n yyc3-platform-production deployment/postgres-master -- pg_dump -U email_user email_platform > backup-$(date +%Y%m%d-%H%M%S).sql
        
        # 文件备份
        kubectl exec -n yyc3-platform-production deployment/minio -- mc mirror minio/yyc3-platform-files /backup/yyc3-platform-files-$(date +%Y%m%d-%H%M%S)/
    ...
    - name: Deploy to production
      run: |
        ...
        # 滚动更新
        kubectl set image deployment/backend-service backend=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}/backend:latest -n yyc3-platform-production
        kubectl set image deployment/frontend-service frontend=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}/frontend:latest -n yyc3-platform-production
        
        # 等待滚动更新完成
        kubectl rollout status deployment/backend-service -n yyc3-platform-production --timeout=600s
        kubectl rollout status deployment/frontend-service -n yyc3-platform-production --timeout=600s

    - name: Verify deployment
      run: |
        # 健康检查
        kubectl get pods -n yyc3-platform-production
        
        # 服务检查
        kubectl get svc -n yyc3-platform-production
  ...
  - name: yyc3-platform.rules
    rules:
      ...
      - alert: BackendServiceDown
        expr: up{job="yyc3-mail-backend"} == 0
        for: 1m
        labels:
          severity: critical
          team: backend
      ...
      - alert: HighBackendResponseTime
        expr: histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{job="yyc3-mail-backend"}[5m])) > 1
        for: 5m
        labels:
          severity: warning
          team: backend
      ...
      - alert: HighErrorRate
        expr: rate(http_requests_total{job="yyc3-mail-backend",status=~"5.."}[5m]) / rate(http_requests_total{job="yyc3-mail-backend"}[5m]) > 0.05
        for: 5m
        labels:
          severity: warning
          team: backend
...
### Grafana仪表板
```json
# monitoring/grafana/dashboards/yyc3-platform-overview.json
{
  "dashboard": {
    "id": null,
    ...
    "panels": [
      {
        ...
        "targets": [
          {
            "expr": "up{job=~\"yyc3-mail-backend|postgres|redis\"}",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "rate(http_requests_total{job=\"yyc3-mail-backend\"}[5m])",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "histogram_quantile(0.50, rate(http_request_duration_seconds_bucket{job=\"yyc3-mail-backend\"}[5m]))",
            "legendFormat": "50th percentile"
          },
          {
            "expr": "histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{job=\"yyc3-mail-backend\"}[5m]))",
            "legendFormat": "95th percentile"
          },
          {
            "expr": "histogram_quantile(0.99, rate(http_request_duration_seconds_bucket{job=\"yyc3-mail-backend\"}[5m]))",
      ...
      },
      {
        ...
        "targets": [
          {
            "expr": "rate(http_requests_total{job=\"yyc3-mail-backend\",status=~\"4..|5..\"}[5m]) / rate(http_requests_total{job=\"yyc3-mail-backend\"}[5m]) * 100",
...
kind: NetworkPolicy
metadata:
  name: yyc3-platform-network-policy
  namespace: yyc3-platform
spec:
  podSelector:
    matchLabels:
...
kind: RoleBinding
metadata:
  name: yyc3-platform-rolebinding
  namespace: yyc3-platform
subjects:
- kind: ServiceAccount
  name: yyc3-platform-sa
...
# 配置变量
BACKUP_BUCKET="s3://yyc3-platform-backups"
KUBECONFIG_CONTEXT="yyc3-platform-production"
...
kubectl --context=$KUBECONFIG_CONTEXT get all -n yyc3-platform > current-state-$(date +%Y%m%d-%H%M%S).txt

# 3. 停止应用服务
echo "3. Stopping application services..."
kubectl --context=$KUBECONFIG_CONTEXT scale deployment backend-service --replicas=0 -n yyc3-platform
kubectl --context=$KUBECONFIG_CONTEXT scale deployment frontend-service --replicas=0 -n yyc3-platform

# 4. 备份当前数据库
echo "4. Backing up current database..."
kubectl exec -n yyc3-platform deployment/$DATABASE_SERVICE -- \
    pg_dump -U email_user email_platform > \
    emergency-backup-$(date +%Y%m%d-%H%M%S).sql

# 5. 清理现有数据（如果需要完全重建）
echo "5. Cleaning existing data..."
kubectl delete pvc -n yyc3-platform --all || true
...
aws s3 cp $BACKUP_BUCKET/latest/$LATEST_BACKUP - | \
    kubectl exec -i -n yyc3-platform deployment/$DATABASE_SERVICE -- \
    psql -U email_user -d email_platform

# 7. 恢复Redis数据
echo "7. Restoring Redis data..."
aws s3 cp $BACKUP_BUCKET/latest/redis-dump.rdb - | \
    kubectl exec -i -n yyc3-platform statefulset/redis-cluster -- \
    redis-cli --pipe

# 8. 重启服务
echo "8. Restarting services..."
kubectl --context=$KUBECONFIG_CONTEXT scale deployment backend-service --replicas=3 -n yyc3-platform
kubectl --context=$KUBECONFIG_CONTEXT scale deployment frontend-service --replicas=2 -n yyc3-platform

# 9. 验证恢复
echo "9. Verifying recovery..."
sleep 30
kubectl --context=$KUBECONFIG_CONTEXT get pods -n yyc3-platform
kubectl --context=$KUBECONFIG_CONTEXT get svc -n yyc3-platform
...
curl -X POST "https://hooks.slack.com/..." \
    ...
    --data '{"text":"✅ Email Platform disaster recovery completed successfully!"}'
```
