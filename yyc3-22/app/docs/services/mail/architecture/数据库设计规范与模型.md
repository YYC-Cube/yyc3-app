...
**数据库类型**: PostgreSQL 15.3+  
**设计原则**: 领域驱动设计 (DDD) + 第三范式 (3NF)  
**一致性策略**: ACID事务保证  
**扩展策略**: 读写分离 + 分库分表
...
│                      应用程序层                              │
├─────────────────────────────────────────────────────────────┤
│  读写分离层 (Read/Write Splitting)                         │
├─────────────────────────────────────────────────────────────┤
│  主库 (Write)    │    从库 (Read x3)                       │
│  PostgreSQL-1    │    PostgreSQL-2,3,4                    │
├─────────────────────────────────────────────────────────────┤
│              数据分片层 (Sharding)                          │
│  用户数据    │    邮件数据    │    分析数据    │    审计数据  │
...
CREATE TABLE emails (
    ...
    -- 收件人信息
    recipients JSONB NOT NULL, -- 支持多个收件人
    -- 格式: {"to": ["user1@example.com"], "cc": [], "bcc": []}
...
CREATE TABLE email_analysis (
    ...
    -- 内容提取
    key_information JSONB, -- 关键信息提取
    -- 格式: {"dates": ["2024-01-01"], "people": [], "locations": [], "actions": []}
    ...
    spam_factors JSONB, -- 垃圾邮件判断因子
    
    -- 智能回复建议
    reply_suggestions JSONB, -- 回复建议
    -- 格式: [{"text": "建议回复内容", "tone": "formal", "confidence": 0.85}]
...
const cacheConfig = {
  ...
  ttl: {
    ...
    emailDetail: 1800,        // 30分钟
    emailStats: 900,          // 15分钟
    analyticsData: 300,       // 5分钟
    systemLogs: 60,           // 1分钟
  },
...
│   ├── 2024_01_15_0002_create_emails_table.sql
│   ├── 2024_01_16_0003_add_email_analysis.sql
│   └── 2024_01_17_0004_create_statistics_views.sql
├── seeds/
│   ├── 001_admin_user.sql
...
数据库设计是系统稳定运行的基础。通过合理的数据模型设计、完善的索引策略和严格的数据完整性保证，确保系统能够高效处理大量邮件数据。
